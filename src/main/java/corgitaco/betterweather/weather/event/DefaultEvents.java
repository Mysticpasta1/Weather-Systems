...
public class DefaultEvents {
    ...
    public static final Map<EntityCheck, Float> DEFAULT_ENTITY_DAMAGE = Util.make(new HashMap<>(), (map) -> {
        map.put(new EntityCheck.CategoryCheck(MobCategory.MONSTER), 0.5F);
        map.put(new EntityCheck.CategoryCheck(MobCategory.AMBIENT), 0.5F);
        map.put(new EntityCheck.CategoryCheck(MobCategory.CREATURE), 0.5F);
        map.put(new EntityCheck.TypeCheck(EntityType.PLAYER.builtInRegistryHolder().key()), 0.5F);
    });
    ...
    public static final Snow DUSTING = new Snow(
            ...
            new Weather.BasicSettings(DEFAULT_BIOME_CONDITION, 0.1D, !MODIFY_TEMPERATURE ? 0.0 : -0.5, 0.1, false, 0, createEffectMap(EntityType.PLAYER, MobEffects.MOVEMENT_SLOWDOWN)),
    ...
    public static final Snow SNOW = new Snow(
            new SnowClientSettings(new ColorSettings(Integer.MAX_VALUE, 0.0, Integer.MAX_VALUE, 0.0), 0.0F, 0.2F, false, Textures.SNOW_LOCATION, SnowClientSettings.SoundSetting.EMPTY),
            new Weather.BasicSettings(DEFAULT_BIOME_CONDITION, 0.1D, !MODIFY_TEMPERATURE ? 0.0 : -0.5, 0.1, false, 0,
                    createEffectMap(EntityType.PLAYER, MobEffects.MOVEMENT_SLOWDOWN)),
    ...
    public static final Snow THUNDER_BLIZZARD = new Snow(
            ...
            new Weather.BasicSettings(DEFAULT_BIOME_CONDITION, 0.05D, !MODIFY_TEMPERATURE ? 0.0 : -0.5, 0.1, true, 100000, createEffectMap(EntityType.PLAYER, MobEffects.MOVEMENT_SLOWDOWN)),
            Weather.DecaySettings.NONE, 2, 10, Blocks.SNOW, true, true);
    ...
    private static Map<EntityCheck, List<Weather.Effect>> createEffectMap(EntityType type, MobEffect effect) {
        return Util.<Map<EntityCheck, List<Weather.Effect>>>make(new HashMap<>(), ((stringListHashMap) -> stringListHashMap.put(new EntityCheck.TypeCheck(type.builtInRegistryHolder().key()), ImmutableList.of(Weather.Effect.from(effect)))));
    }
}
